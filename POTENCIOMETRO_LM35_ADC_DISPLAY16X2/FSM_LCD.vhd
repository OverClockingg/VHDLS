                ...
                when ST_FETCH =>
                    ...
                    if msg_id = "0100" then 
                        case r_index is
							...
							elsif msg_id = "0101" then      -- Lógica para POTENCIÓMETRO (Ángulo)
                        case r_index is
                            when 9  => r_data <= ascii_centenas;
									 when 10 => r_data <= ascii_decenas;
									 when 11 => r_data <= ascii_unidades;
								    when 12 => r_data <= x"DF"; -- Símbolo de grado ° en la posición 12
                            when others => r_data <= get_lcd_char(msg_id, r_index);
                        end case;	
